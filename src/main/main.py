...
from src.main.perception_layer.clothing_detection_rf_detr.inference import GarmentDetector
from src.main.perception_layer.multi_attribute_classifier.old_model.inference import AttributePredictor
from src.main.representation_layer.visual_embeddings.inference import GarmentEmbedder
from src.main.recomendation_engine.brain_engine import FashionBrain
from src.main.perception_layer.color_utils import quantize_colors, harmony_score_from_images
from src.main.semantic_processing.query_vectorization.query_vectorizer import QueryVectorizer
from src.main.semantic_processing.intent_filter_extractor.IntentExtractor import IntentExtractor
import src.config as cfg

# --- Configuration & Paths ---
UPLOAD_DIR = cfg.PUBLIC_DIR / "uploads"
...
async def lifespan(app: FastAPI):
    ...
    print(f"Models will be loaded from {cfg.MODELS_DIR}")
    if not cfg.MODELS_DIR.exists():
        ...
        raise FileNotFoundError(f"Model directory not found: {cfg.MODELS_DIR}")

    # 1. Perception
    print("Loading RF-DETR Detector...")
    RF_DETR_MODEL_PATH = cfg.RF_DETR_MODEL_DIR / "checkpoint_best_ema.pth"
    ml_models['detector'] = GarmentDetector(str(RF_DETR_MODEL_PATH))


    ATTR_MODEL_PATH = cfg.ATTRIBUTE_PREDICTOR_DIR / "1b" / "best_model.pt"
    ...
    EMBED_MODEL_PATH = cfg.VISUAL_EMBEDDER_DIR / "best_student_model.pth"
    ml_models['garment_embedder'] = GarmentEmbedder(str(EMBED_MODEL_PATH), device=DEVICE)

    # 3. Semantic
    print("Loading NLP Models...")
    NLP_MODEL_PATH = cfg.NLP_MODEL_DIR / "distilled_query_encoder.pth"
    ...
    STYLIST_MODEL_PATH = cfg.STYLIST_BRAIN_DIR / "best_stylist.pth"
