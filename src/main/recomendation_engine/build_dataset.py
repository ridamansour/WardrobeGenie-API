...
from src.main.perception_layer import color_utils
from src.config import VISUAL_EMBEDDER_DIR, FASHIONOPEDIA_COCO_DIR
...
def process_fashionpedia(json_path, img_dir, output_file="processed_pool.pt"):
    ...
    """
    embedder = GarmentEmbedder(VISUAL_EMBEDDER_DIR / "best_student_model.pth", device="cuda")
...
if __name__ == "__main__":
    process_fashionpedia(
        json_path=FASHIONOPEDIA_COCO_DIR / "train" /" _annotations.coco.json",
        img_dir=FASHIONOPEDIA_COCO_DIR / "train",
        output_file=FASHIONOPEDIA_COCO_DIR / "processed_pool.pt"
    )